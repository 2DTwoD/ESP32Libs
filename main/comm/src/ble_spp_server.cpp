...
ble_svc_gatt_handler(uint16_t conn_handle, uint16_t attr_handle, struct ble_gatt_access_ctxt *ctxt, void *arg) {
    switch (ctxt->op) {
        ...
        case BLE_GATT_ACCESS_OP_WRITE_CHR:
            MODLOG_DFLT(INFO, "Data received in write event,conn_handle = %x,attr_handle = %x", conn_handle,
                        attr_handle);
            break;
...
static ble_uuid16_t uuid1{
        {BLE_UUID_TYPE_16},
        BLE_SVC_SPP_UUID16
};
static ble_uuid16_t uuid2{
        {BLE_UUID_TYPE_16},
...
static const struct ble_gatt_svc_def new_ble_svc_gatt_defs[] = {
        {
                ...
                .uuid = (ble_uuid_t *) &uuid1,
                .includes{},
                .characteristics = (struct ble_gatt_chr_def[])
                        {{
                                 /* Support SPP service */
                                 .uuid = (ble_uuid_t *) &uuid2,
...
go_ble_spp(void) {
    ...
    rc = ble_svc_gap_device_name_set("ESP32bleCOM");
    assert(rc == 0);
