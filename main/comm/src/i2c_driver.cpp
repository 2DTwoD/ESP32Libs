...
I2CDriver::I2CDriver(i2c_port_t port, uint8_t sclPin, uint8_t sdaPin, int32_t timeout = -1, bool pullUp = true) {
    if(init({port,
          (gpio_num_t)sclPin,
          (gpio_num_t)sdaPin,
          pullUp
...
CommStatus I2CDriver::addSlave(uint8_t address, uint32_t speed,i2c_addr_bit_len_t addrLen = I2C_ADDR_BIT_LEN_7) {
